...
import { defineNuxtModule, addPluginTemplate, extendPages } from '@nuxt/kit';
...
function routeHook(
  ...
  try {
    extendPages((routes: NuxtRouteConfig[]) => {
      ...
      const recursiveTypedRoutes = (
        ...
        if (
          ...
          params?.shift();
          let allMergedParams: ParamDecl[] = params.map((m) => ({
            key: m,
            type: 'string | number',
            required: true,
          }));
      ...
      * */`;
          ...
          .map(
            ({ name, params }) =>
              `"${name}": ${
                params.length
                  ? `{
              ${params.map((p) => `"${p.key}"${p.required ? '' : '?'}: ${p.type}`).join(',\n')}
            }`
                  : 'never'
              }`
          )
      ...
      export type RouteListDecl = ${routesObjectDecl};
...
export default defineNuxtModule<NuxtTypedRouterOptions>({
  ...
  setup(moduleOptions, nuxt) {
    const {
      filePath = `${nuxt.options.srcDir}/__routes.ts`,
    ...
    nuxt.hook('build:before', () =>
      routeHook(filePath, routesObjectName, stripAtFromName, definitionFilePath)
    );
