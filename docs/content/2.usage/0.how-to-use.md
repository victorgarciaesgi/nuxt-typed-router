---
title: How to use
description: Learn how Nuxt Typed Router works with different page structures
navigation:
...
The module will automatically provide autocomplete and type-check for route `path`, `name` and `params`.

Params autocomplete takes into account the way vue-router works (navigating between children routes without re-providing the param).

The types from `vue-router` and Nuxt navigation utils will be overwritten, allowing for safe navigation.

# Examples for different pages structures

All those examples are about files present in the `pages` folder.
...
::note
With `useRoute`, there is an additional guard that can save you some unnecessary check if you're confident on the page you're in. See [useRoute docs](/usage/useRoute) for more info.
::
...
When you define a "catch-all" route, navigating exactly to this route will require to provide a `params` property, and will need the param to be an `Array`.
...
Because of how `vue-router` behaves, you are not required to re-provide parent params.

For example, navigating from route `pages/nested/[title]/user/[id]/posts.vue` to `pages/nested/[title]/user/[id]/index.vue`, as `[id]` is a required param, you will not have to pass `title` and `id` params.
...
As for resolved routes, the parameters will not be marked as optional:
